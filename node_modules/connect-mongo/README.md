...
// Basic usage
server.use(session({
  store: MongoStore.create({ mongoUrl: 'mongodb://localhost/test-server' })
}));

// Advanced usage
server.use(session({
  store: MongoStore.create({
    mongoUrl: 'mongodb://user12345:foobar@localhost/test-server?authSource=admin&w=1',
...
server.use(session({
  store: MongoStore.create({
    mongoUrl: 'mongodb://localhost/test-server',
...
|`unserialize`||Custom hook for unserializing sessions from MongoDB. This can be used in scenarios where you need to support different types of serializations (e.g., objects and JSON strings) or need to modify the session before using it in your server.|
